...
export default function StatsCards({ runs }: Props) {
  const actualRuns = runs.filter((r) => r.status !== "planned");
  const now = new Date();
  ...
  const weekRuns = actualRuns.filter((r) => new Date(r.date) >= weekStart);
  const monthRuns = actualRuns.filter((r) => new Date(r.date) >= monthStart);
  ...
  const allKm = sumDistance(actualRuns).toFixed(2);
  const longest = Math.max(0, ...actualRuns.map((r) => r.distanceKm)).toFixed(
    2
  );
  const count = actualRuns.length;
